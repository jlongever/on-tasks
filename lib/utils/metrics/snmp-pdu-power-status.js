...
function snmpPduPowerMetricFactory(
    ...
    SnmpPduPowerMetric.prototype.calculatePowerData = function(values) {
        ...
        if (this.nodeType === 'sinetica') {
            ...
            .then(function(results) {
                // map and traverse the result object to get to the nested outlets object
                // this would be easier if outlets was a list instead of an object
                if(_.has(self.data.cache, self.data.workItemId)) {
                    var data = self.data.cache[self.data.workItemId];
                    if(_.has(data, self.data.config.metric)) {
                        var lastResults = _(data[self.data.config.metric])
                        .map(function(pdu) {
                             return _.map(pdu.outlets, function(outlet) {
                                return outlet;   
                            });                       
                        }).first();
                        ...
                        lastResults = _.sortBy(lastResults, 'pduOutName');
                        currentResults = _.sortBy(currentResults, 'pduOutName');
                        var status = _.reduce(lastResults, function(result, last, i) {
                            if(last.pduOutOn !== currentResults[i].pduOutOn) {
                                result.push({
                                    outlet: currentResults[i].pduOutName,
                                    current: currentResults[i].pduOutOn.toUpperCase(),
                                    last: last.pduOutOn.toUpperCase()
                                });
                            }
                            return result;
                        }, []);
    ...
    SnmpPduPowerMetric.prototype._calculateSineticaPowerData = Promise.method(function(powerData) {
        var out = {};
        var unitsMap = { 
            'pduOutName': 'null',
            'pduOutOn': 'null',
